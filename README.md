...
the wlRIX Avalonia theme and dialogs, and a **patched `Avalonia.Wayland`**. It is gitignored, so a
fresh clone has to rebuild it before `dotnet restore` will work.
...
- **`CanResize=false` on the wire.** Mapped onto `min == max` size constraints, which is the only
  spelling xdg-shell has for "this window is a fixed size". `wlrix-compositor` reads it to drop the
  maximize button and the resize grips (see its README, *Window capabilities*). `SetCanMaximize`
...
Two things then need fixing up by hand, because upstream's own packages are assembled by Nuke
(`numerge.json`) rather than by `dotnet pack`, and running the whole Nuke pipeline for one library
is not worth it:
...
2. Those versions come out matching `PackageVersion` rather than the source version. They must say
   **12.1.0** — the tag the code is built from, and what the assembly references resolve to.

Then drop the `.nupkg` into `localfeed/`, delete the version it replaces, and bump
`Avalonia.Wayland` in `Directory.Packages.props` to match. Bump the `-wlrix.N` suffix on every
rebuild: NuGet caches by id and version, so reusing a version means the old one is served from
